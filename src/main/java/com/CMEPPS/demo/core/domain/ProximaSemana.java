...
public class ProximaSemana {
	...
	public ProximaSemana(String userName) {
	...
	public ProximaSemana(String userName, ArrayList<Todo> listaTodos, ArrayList<Horario> listaHorarios,
			ArrayList<Reparto> listaReparto) {
		this.userName = userName;
		setTodosUsuario(listaTodos);
		setReparto(listaReparto);
	...
	private int indexRepartoTodo(Todo todo) {
		for (int i = 0; i < reparto.size(); i++) {
			...
			if (obtenerTododeReparto(i).getDescription().equals(todo.getDescription())) {
				return i;
			}
		}
		return -1;
